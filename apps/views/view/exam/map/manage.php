		...
		if(count($kmList)>0):?>
			<?php foreach ($kmList as $value): ?>
				<li id="li-<?php echo $value->id; ?>">
					<div class="delBtn" onclick="hideKm('<?php echo $value->id; ?>')">x</div>
					<span>科目：<?php echo $value->subjectName; ?></span>
					<span>年級：
					...
					<?php elseif($value->grade>13 && $value->grade<=17): ?>
	                <?php echo "大學 ".($value->grade-9)." 年級" ?>
					<?php endif; ?>
					</span>
					<span>
						建立者：<?php echo $value->teacherName; ?>
					</span>
					<div class="btnList">
						<span class="blueBtn" onclick="openKm('<?php echo $value->id; ?>')">查看</span>
						<span class="blueBtn" onclick="examManage('<?php echo $value->id; ?>')">試卷管理</span>
					</div>
				</li>
			<?php endforeach; ?>
		<?php else: ?>
